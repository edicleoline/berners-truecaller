...
val retrofit = "2.9.0"
val moshi = "1.8.0"
val accompanist = "0.24.11-rc"
val navigationCompose = "2.4.2"
val landscapistGlide = "1.5.2"
...
dependencies {
    constraints {
        ...
        api("${Libs.MOSHI_ADAPTERS}:$moshi")
        api("${Libs.MOSHI_CODEGEN}:$moshi")
        api("${Libs.MOSHI_KOTLIN}:$moshi")
        api("${Libs.RETROFIT_MOSHI_CONVERTER}:$retrofit")

        api("${Libs.ACCOMPANIST_SWIPEREFRESH}:$accompanist")
        ...
        api("${Libs.COMPOSE_UI_UTIL}:$compose")
        api("${Libs.COMPOSE_PAGING}:$composePaging")

        api("${Libs.NAVIGATION_COMPOSE}:$navigationCompose")
        api("${Libs.LANDSCAPIST_GLIDE}:$landscapistGlide")

        api("${Libs.KOTLINX_DATETIME}:$kotlinxDatetime")
        api("${Libs.PRETTYTIME}:$prettytime")
    }
}
